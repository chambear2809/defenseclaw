...
Transport, credentials, and signals are explicit per destination. During
upgrade, a legacy flat exporter in `otel.*` or the documented
...
the legacy route. The compatibility `otel.galileo` block can add one or more
Galileo trace fan-out exporters without replacing named OTLP destinations.
