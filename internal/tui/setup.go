...
func ciscoAIDefenseFields(c *config.Config) []configField {
	...
	return []configField{
		...
		{Label: "OAuth Token URL", Key: "cisco_ai_defense.oauth_token_url", Kind: "string", Value: c.CiscoAIDefense.OAuthTokenURL,
			Hint: "OAuth client-credentials token endpoint used when no API key is configured."},
		{Label: "OAuth Basic Env", Key: "cisco_ai_defense.oauth_basic_env", Kind: "string", Value: c.CiscoAIDefense.OAuthBasicEnv,
			Hint: "Env var NAME holding the Basic credential for OAuth token exchange."},
		{Label: "OAuth Basic (redacted)", Key: "cisco_ai_defense.oauth_basic", Kind: "password", Value: c.CiscoAIDefense.OAuthBasic,
			Hint: "Inline OAuth Basic credential. Prefer OAuth Basic Env so secrets stay out of config.yaml."},
