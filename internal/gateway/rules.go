...
var commandRules = []PatternRule{
	...
	{ID: "CMD-KUBECTL-DELETE-ALL", Pattern: regexp.MustCompile(`(?i)(?:\bkubectl\b[^\n{}]*(?:-n\s+|--namespace(?:=|\s+))(?:defenseclaw|defenesclaw|kube-system|default)\b[^\n{}]*\bdelete\b|\bkubectl\b[^\n{}]*\bdelete\b[^\n{}]*(?:--all\b|\bnamespaces?\b|\ball\b|(?:-n\s+|--namespace(?:=|\s+))(?:defenseclaw|defenesclaw|kube-system|default)\b))`), Title: "Destructive Kubernetes delete", Severity: "CRITICAL", Confidence: 0.88, Tags: []string{"destructive", "kubernetes"}},
	{ID: "CMD-KUBECTL-SECRET-DUMP", Pattern: regexp.MustCompile(`(?i)\bkubectl\b[^\n{}]*\bget\b[^\n{}]*\bsecrets?\b[^\n{}]*(?:-o\s+(?:yaml|json)\b|--output(?:=|\s+)(?:yaml|json)\b|--all-namespaces\b|\s-A\b)`), Title: "Kubernetes secret value dump", Severity: "CRITICAL", Confidence: 0.90, Tags: []string{"credential", "kubernetes"}},
