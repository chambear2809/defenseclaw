...
func TestOTelConfigValidateAllowsGalileoOnlyExporter(t *testing.T) {
	cfg := OTelConfig{
		Enabled: true,
		Galileo: OTelGalileoConfig{
			Enabled:     true,
			ProjectID:   "project-id",
			LogStreamID: "log-stream-id",
		},
	}
	if err := cfg.ValidateNamedDestinations(); err != nil {
		t.Fatalf("Galileo-only OTel config rejected: %v", err)
	}
}
