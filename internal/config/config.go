...
type OTelConfig struct {
	...
	Galileo      OTelGalileoConfig       `mapstructure:"galileo"      yaml:"galileo,omitempty"`
}
...
func (c OTelConfig) ValidateNamedDestinations() error {
	if c.Enabled && len(c.Destinations) == 0 && len(c.Galileo.ExporterConfigs()) == 0 {
...
func setDefaults(dataDir string) {
	...
	// User-session OS notifications. Master switch defaults to false
	// on every platform — see DefaultNotificationsEnabled in
	// notifications.go for the rationale. block_enforced and
