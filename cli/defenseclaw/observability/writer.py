...
def _migrate_flat_otel_in_place(
    ...
    *,
    ...
    first_party_signal_endpoint_present = _runtime_env_is_set(
        data_dir,
        "DEFENSECLAW_OTEL_TRACES_ENDPOINT",
        "OPENCLAW_OTEL_TRACES_ENDPOINT",
        "DEFENSECLAW_OTEL_METRICS_ENDPOINT",
        "OPENCLAW_OTEL_METRICS_ENDPOINT",
        "DEFENSECLAW_OTEL_LOGS_ENDPOINT",
        "OPENCLAW_OTEL_LOGS_ENDPOINT",
    )
    global_endpoint_names = ["DEFENSECLAW_OTEL_ENDPOINT", "OPENCLAW_OTEL_ENDPOINT"]
    global_protocol_names = ["DEFENSECLAW_OTEL_PROTOCOL", "OPENCLAW_OTEL_PROTOCOL"]
    if not first_party_signal_endpoint_present:
        global_endpoint_names.append("OTEL_EXPORTER_OTLP_ENDPOINT")
        global_protocol_names.append("OTEL_EXPORTER_OTLP_PROTOCOL")
    env_global_endpoint = _first_runtime_env(data_dir, *global_endpoint_names)
    env_signal_endpoints = {
        sig: _first_runtime_env(data_dir, *_signal_endpoint_env_names(sig, first_party_signal_endpoint_present))
        for sig in ("traces", "metrics", "logs")
    }
    env_signal_protocols = {
        sig: _first_runtime_env(data_dir, *_signal_protocol_env_names(sig, first_party_signal_endpoint_present))
        for sig in ("traces", "metrics", "logs")
    }
    ...
    destination: dict[str, Any] = {
        ...
        "protocol": str(
            otel.get("protocol", "")
            or _first_runtime_env(data_dir, *global_protocol_names)
...
def _runtime_env_is_set(data_dir: str, *names: str) -> bool:
    dotenv = _load_dotenv(os.path.join(data_dir, DOTENV_FILE_NAME))
    return any(name in os.environ or name in dotenv for name in names)


def _signal_endpoint_env_names(signal: str, first_party_only: bool) -> list[str]:
    upper = signal.upper()
    names = [f"DEFENSECLAW_OTEL_{upper}_ENDPOINT", f"OPENCLAW_OTEL_{upper}_ENDPOINT"]
    if not first_party_only:
        names.append(f"OTEL_EXPORTER_OTLP_{upper}_ENDPOINT")
    return names


def _signal_protocol_env_names(signal: str, first_party_only: bool) -> list[str]:
    upper = signal.upper()
    names = [f"DEFENSECLAW_OTEL_{upper}_PROTOCOL", f"OPENCLAW_OTEL_{upper}_PROTOCOL"]
    if not first_party_only:
        names.append(f"OTEL_EXPORTER_OTLP_{upper}_PROTOCOL")
    return names
